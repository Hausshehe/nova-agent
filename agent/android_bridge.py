...
class AndroidBridge:
    ...
    needed, must be implemented as a separate explicitly-invoked capability.
    """
    ...
    def health(self) -> dict[str, Any]:
        """Return bridge/accessibility health without requiring root privileges."""
    ...
    def launch(self, package: str = "com.hausshehe.nova", root: bool = False) -> dict[str, Any]:
        """Launch Nova without root; ``root`` is retained only for API compatibility.

        The flag is intentionally ignored. Normal navigation must never invoke
        ``su`` and therefore must never trigger a Magisk authorization prompt.
        """
        try:
