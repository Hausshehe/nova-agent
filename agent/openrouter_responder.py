...
Keep the plan focused on the goal and current evidence. Do not invent UI ids,
coordinates, commands, or actions. The runtime validates and bounds the plan."""
_RESPONSE_FORMATS = {"reasoning": {"type": "json_object"}, "planning": {"type": "json_object"}}
...
class OpenRouterResponder:
    ...
    def __call__(self, prompt: str) -> Mapping[str, Any]:
        ...
        payload = {"model": self._model, "messages": [{"role": "user", "content": f"{instruction}\n\nLive Nova context:\n{prompt}"}], "temperature": 0, "max_tokens": DEFAULT_MAX_TOKENS, "response_format": _RESPONSE_FORMATS[self._task], "stream": False}
        body = json.dumps(payload, ensure_ascii=False).encode("utf-8")
        req = request.Request(OPENROUTER_CHAT_COMPLETIONS_URL, data=body, headers={"Authorization": f"Bearer {self._api_key}", "Content-Type": "application/json", "User-Agent": USER_AGENT, "Accept": "application/json", "Connection": "close"}, method="POST")
        try:
        ...
        try:
            ...
            raise RuntimeError("OpenRouter response missing choices/message: " + f"{json.dumps(envelope, ensure_ascii=False)[:1200]}") from exc
        if not isinstance(content, str) or not content.strip():
            raise RuntimeError("OpenRouter returned no usable reasoning content")
        try:
