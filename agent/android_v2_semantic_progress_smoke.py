...
def _action_responder(prompt: str, bridge: AndroidBridge):
    ...
    if "fallback" in current.casefold():
        ...
        if observation.get("package") != PACKAGE_NAME:
            # The smoke itself starts from Termux. If Android unexpectedly
            # restores that foreground task during a multi-step mission,
            # recover the expected app instead of ever acting on the foreign
            # accessibility tree. This is test-harness recovery, not a core
            # runtime assumption about which package a real mission targets.
            bridge.launch(PACKAGE_NAME, root=False)
            return {
                "action_type": "wait",
                "target_id": None,
                "reason": f"relaunching expected package after foreign foreground: {observation.get('package')!r}",
            }
        scroll_debug = [
...
def main() -> int:
    ...
    runtime = build_capability_runtime(
        ...
        action_responders=[("cerebras", lambda prompt: _action_responder(prompt, bridge))],
