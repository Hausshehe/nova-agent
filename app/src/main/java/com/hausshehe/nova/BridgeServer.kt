...
object BridgeServer {
    ...
    private fun scroll(elementId: String): JSONObject {
        ...
        val root = service.rootInActiveWindow ?: return error("No active accessibility window")
        val node = findNode(root, elementId) ?: return error("element not found: $elementId")
        val accepted = node.isEnabled && node.isScrollable && node.performAction(AccessibilityNodeInfo.ACTION_SCROLL_FORWARD)
        node.recycle()
        return JSONObject().apply {
            put("ok", true)
            put("accepted", accepted)
    ...
    private fun back(): JSONObject {
        ...
        return JSONObject().apply { put("ok", true); put("accepted", accepted); put("changed", accepted) }
    }
    ...
    private fun error(message: String): JSONObject = JSONObject().apply { put("ok", false); put("error", message) }
}
