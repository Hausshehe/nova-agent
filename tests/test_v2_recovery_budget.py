...
def test_rejected_execution_does_not_consume_action_budget():
    ...
    controller.move(controller.state.OBSERVING)
    controller.record_observation(_observation(1))
    controller.move(controller.state.DECIDING)
    controller.record_decision(Decision(Action(ActionType.TAP, target_id="bad")))
    controller.move(controller.state.EXECUTING)
...
def test_runtime_recovers_from_rejection_without_losing_the_action_slot():
    ...
    verifier = Verifier()
    runtime = Runtime(
        Goal("finish"), observer, reasoner, executor, verifier, max_steps=3
    )
