...
class LLMPlanner:
    ...
    def _request(self, context: ReasoningContext, previous: Plan | None) -> str:
        ...
        prompt = (
            ...
            "intent MUST advance one of them. High uncertainty means reassess and resolves an unknown; do not repeat "
            "an ineffective action without new evidence. Medium means choose a verifiable action whose effect can be "
            "verified from a fresh observation; low means proceed normally. Conflicting evidence must remain unresolved "
            "until current evidence supports a resolution.\n"
